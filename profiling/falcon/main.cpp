...
falcon_keygen_make(
	...
	uint64_t *psm, uint64_t *pss, uint64_t *pstf, 
	uint64_t *fft, uint64_t *pif, uint64_t *paf, 
	uint64_t *pm, uint64_t *pmaf, uint64_t *ifft, 
	uint64_t *bfa, uint64_t *cpk, uint64_t *sn,
	uint64_t *epk, uint64_t *rpkaei)
{
	int8_t *f, *g, *F;
	uint16_t *h;
	uint8_t *atmp;
...
do_sign_dyn(samplerZ samp, void *samp_ctx, int16_t *s2,
	...
	uint64_t ****cbtfft, uint64_t **** cgsm, uint64_t **** alb,
	uint64_t **** ffsamp, uint64_t **** recompmat, uint64_t **** getlatp)
{
	size_t n, u;
	fpr *t0, *t1, *tx, *ty;
...
{ 
	...
	for (;;) {
		...
		if (do_sign_dyn(samp, samp_ctx, sig,
			...
			&cbtfft, &cgsm, &alb, &ffsamp, &recompmat, &getlatp))
		{
			break;
		}
	}
...
falcon_sign_dyn_finish(shake256_context *rng,
	...
	void *tmp, size_t tmp_len,
	uint64_t **senc, uint64_t **dpkcpk, uint64_t **hmtap,
	uint64_t **cbtfft, uint64_t ** cgsm, uint64_t ** alb, 
	uint64_t ** ffsamp, uint64_t ** recompmat, uint64_t ** getlatp)
{
	unsigned logn;
...
falcon_expand_privkey(void *expanded_key, size_t expanded_key_len,
	const void *privkey, size_t privkey_len,
	void *tmp, size_t tmp_len,
	uint64_t *getdegsk, uint64_t *decsk, uint64_t *expsk)
{
	unsigned logn;
	const uint8_t *sk;
	int8_t *f, *g, *F, *G;
...
do_sign_tree(samplerZ samp, void *samp_ctx, int16_t *s2,
	...
	const uint16_t *hm, unsigned logn, fpr * tmp,
	uint64_t ****ffsfft, uint64_t ****albt, 
	uint64_t ****getlatpt, uint64_t ****compsigt)
{
	size_t n, u;
	fpr *t0, *t1, *tx, *ty;
...
falcon_sign_tree_finish(shake256_context *rng,
	...
	shake256_context *hash_data, const void *nonce,
	void *tmp, size_t tmp_len,
	uint64_t **senct, uint64_t **dpkcpkt, uint64_t **hmtapt,
	uint64_t **ffsfft, uint64_t **albt, uint64_t **getlatpt,
	uint64_t **compsigt)
{
	unsigned logn;
	uint8_t *es;
...
falcon_sign_dyn(shake256_context *rng,
	...
	void *tmp, size_t tmp_len,
	uint64_t *fss, uint64_t *senc, uint64_t *dpkcpk, uint64_t *hmtap,
	uint64_t *cbtfft, uint64_t * cgsm, uint64_t * alb, uint64_t * ffsamp, 
	uint64_t * recompmat, uint64_t * getlatp)
{
	...
	return falcon_sign_dyn_finish(rng, sig, sig_len, sig_type,
		...
		&senc, &dpkcpk, &hmtap, &cbtfft, &cgsm, &alb, 
		&ffsamp, &recompmat, &getlatp);
}
...
falcon_sign_tree(shake256_context *rng,
	void *sig, size_t *sig_len, int sig_type,
	const void *expanded_key, const void *data, 
	size_t data_len, void *tmp, size_t tmp_len,
	uint64_t *senct, uint64_t *fsst, uint64_t *dpkcpkt, uint64_t *hmtapt,
	uint64_t *ffsfft, uint64_t *albt, uint64_t *getlatpt, uint64_t *compsigt)
{
	...
	return falcon_sign_tree_finish(rng, sig, sig_len, sig_type,
		...
		&senct, &dpkcpkt, &hmtapt, &ffsfft, &albt, &getlatpt, &compsigt);
}
...
falcon_verify_finish(const void *sig, size_t sig_len, int sig_type,
	...
	uint64_t **getdeg, uint64_t **decpk, uint64_t **decsig, 
	uint64_t **hmtpver, uint64_t **versig)
{
	unsigned logn;
	uint8_t *atmp;
	const uint8_t *pk, *es;
...
falcon_verify(const void *sig, size_t sig_len, int sig_type,
	...
	const void *data, size_t data_len,
	void *tmp, size_t tmp_len,
	uint64_t *vstart, uint64_t *getdeg, uint64_t *decpk, 
	uint64_t *decsig, uint64_t *hmtpver, uint64_t *versig)
{
	...
	return falcon_verify_finish(sig, sig_len, sig_type,
		...
		&getdeg, &decpk, &decsig, &hmtpver, &versig);
}
...
int main()
{
	...
	*/
	...
	void *pubkey, *privkey, *expkey, *sig, *pubkey2;
	...
	uint64_t kg, sg, vf, esk, sgt;
	kg = sg = vf = esk = sgt = 0;

	// keygen function ccounts
	uint64_t psm, pss, pstf, fft, pif, paf, pm, pmaf, ifft, bfa, cpk, sn, epk, rpkaei;
	psm = pss = pstf = fft = pif = paf = pm = pmaf = ifft = bfa = cpk = sn = epk = rpkaei = 0;
	
	// sign dyn function ccounts
	uint64_t senc, fss, dpkcpk, hmtap;
	uint64_t cbtfft, cgsm, alb, ffsamp, recompmat, getlatp;
	senc = fss = dpkcpk = hmtap = cbtfft = cgsm = alb = ffsamp = recompmat = getlatp = 0;
	
	// verify function ccounts
	uint64_t vstart, getdeg, decpk, decsig, hmtpver, versig;
	vstart = getdeg = decpk = decsig = hmtpver = versig = 0;
	
	// expand priv key function ccounts
	uint64_t getdegsk, decsk, expsk;
	getdegsk = decsk = expsk = 0;
	
	// sign tree function ccounts
	uint64_t senct, fsst, dpkcpkt, hmtapt;
	uint64_t albt, ffsampt, recompmatt, getlatpt, ffsfft, compsigt;
	senct = fsst = dpkcpkt = hmtapt = albt = ffsampt = ffsfft = recompmatt = getlatpt = compsigt = 0;
	...
	for (size_t r=0; r<BENCHMARK_ROUND; r++) {
		DWT->CYCCNT = 0;
		CYCLES_START_M
		ret_val += falcon_keygen_make(&sc, logn, privkey, 
			privkey_len, pubkey, pubkey_len, tmpkg, tmpkg_len,
			&psm, &pss, &pstf, &fft, &pif, &paf, &pm, &pmaf, 
			&ifft, &bfa, &cpk, &sn, &epk, &rpkaei);
		CYCLES_ADD_M(kg)
	}
	...
	for (size_t r=0; r<BENCHMARK_ROUND; r++) {
		...
		ret_val += falcon_sign_dyn(&sc, sig, &sig_len, FALCON_SIG_CT,
				...
				&fss, &senc, &dpkcpk, &hmtap, &cbtfft, &cgsm, &alb, 
				&ffsamp, &recompmat, &getlatp);
		CYCLES_ADD_M(sg)
	}

	total = senc + fss + dpkcpk + hmtap + cbtfft + cgsm + alb + ffsamp + recompmat + getlatp;	
	pc.printf("sign start clocks:           %lld (%f%%)\n\r",fss/BENCHMARK_ROUND,(double)((double)fss/(double)total)*100);
	pc.printf("decode/comp priv key clocks: %lld (%f%%)\n\r",dpkcpk/BENCHMARK_ROUND,(double)((double)dpkcpk/(double)total)*100);
	pc.printf("hash mess to point clocks:   %lld (%f%%)\n\r",hmtap/BENCHMARK_ROUND,(double)((double)hmtap/(double)total)*100);
	pc.printf("signature encode clocks:     %lld (%f%%)\n\r",senc/BENCHMARK_ROUND,(double)((double)senc/(double)total)*100);
	pc.printf("convert basis to fft clocks: %lld (%f%%)\n\r",cbtfft/BENCHMARK_ROUND,(double)((double)cbtfft/(double)total)*100);
	pc.printf("comp gram matrix clocks:     %lld (%f%%)\n\r",cgsm/BENCHMARK_ROUND,(double)((double)cgsm/(double)total)*100);
	pc.printf("apply lattice basis clocks:  %lld (%f%%)\n\r",alb/BENCHMARK_ROUND,(double)((double)alb/(double)total)*100);
	pc.printf("ffsampling clocks:           %lld (%f%%)\n\r",ffsamp/BENCHMARK_ROUND,(double)((double)ffsamp/(double)total)*100);
	pc.printf("recomp matrix basis clocks:  %lld (%f%%)\n\r",recompmat/BENCHMARK_ROUND,(double)((double)recompmat/(double)total)*100);
	pc.printf("get lattice point clocks:    %lld (%f%%)\n\r",getlatp/BENCHMARK_ROUND,(double)((double)getlatp/(double)total)*100);
	pc.printf("---------------------------------\n\r");
	pc.printf("total measured clocks:       %lld (%f%%)\n\r", total/BENCHMARK_ROUND,(double)((double)total/(double)total)*100);	
	pc.printf("---------------------------------\n\r");
	pc.printf("total sign d (%%meas) clocks: %lld (%f%%)\n\r", sg/BENCHMARK_ROUND, (double)((double)(total/BENCHMARK_ROUND)/(double)(sg/BENCHMARK_ROUND))*100);
	pc.printf("---------------------------------\n\r");
	...
	for (size_t r=0; r<BENCHMARK_ROUND; r++) {
		...
		ret_val += falcon_verify(sig, sig_len, FALCON_SIG_CT, pubkey, 
				...
				&vstart, &getdeg, &decpk, &decsig, &hmtpver, &versig);
		CYCLES_ADD_M(vf)
	}
	
	total = vstart + getdeg + decpk + decsig + hmtpver + versig;
	
	pc.printf("verf start clocks:           %lld (%f%%)\n\r",vstart/BENCHMARK_ROUND,(double)((double)vstart/(double)total)*100);
	pc.printf("get degree via pk clocks:    %lld (%f%%)\n\r",getdeg/BENCHMARK_ROUND,(double)((double)getdeg/(double)total)*100);
	pc.printf("decode pub key clocks:       %lld (%f%%)\n\r",decpk/BENCHMARK_ROUND,(double)((double)decpk/(double)total)*100);
	pc.printf("decode sign clocks:          %lld (%f%%)\n\r",decsig/BENCHMARK_ROUND,(double)((double)decsig/(double)total)*100);
	pc.printf("hash mess to point clocks:   %lld (%f%%)\n\r",hmtpver/BENCHMARK_ROUND,(double)((double)hmtpver/(double)total)*100);
	pc.printf("verify sign clocks:          %lld (%f%%)\n\r",versig/BENCHMARK_ROUND,(double)((double)versig/(double)total)*100);
	pc.printf("---------------------------------\n\r");
	pc.printf("total measured clocks:       %lld (%f%%)\n\r", total/BENCHMARK_ROUND,(double)((double)total/(double)total)*100);	
	pc.printf("---------------------------------\n\r");
	...
	for (size_t r=0; r<BENCHMARK_ROUND; r++) {
		...
		ret_val += falcon_expand_privkey(expkey, expkey_len,
				...
				&getdegsk, &decsk, &expsk);
		CYCLES_ADD_M(esk)
	}
		
	total = getdegsk + decsk + expsk;
	pc.printf("get priv deg clocks:         %lld (%f%%)\n\r",getdegsk/BENCHMARK_ROUND,(double)((double)getdegsk/(double)total)*100);
	pc.printf("decode priv clocks:          %lld (%f%%)\n\r",decsk/BENCHMARK_ROUND,(double)((double)decsk/(double)total)*100);
	pc.printf("expand priv key clocks:      %lld (%f%%)\n\r",expsk/BENCHMARK_ROUND,(double)((double)expsk/(double)total)*100);
	pc.printf("---------------------------------\n\r");
	pc.printf("total measured clocks:       %lld (%f%%)\n\r", total/BENCHMARK_ROUND,(double)((double)total/(double)total)*100);	
	pc.printf("---------------------------------\n\r");
	pc.printf("total expsk (%%meas) clocks:  %lld (%f%%)\n\r", esk/BENCHMARK_ROUND, (double)((double)(total/BENCHMARK_ROUND)/(double)(esk/BENCHMARK_ROUND))*100);
	pc.printf("---------------------------------\n\r");
	...
	for (size_t r=0; r<BENCHMARK_ROUND; r++) {
		...
		ret_val += falcon_sign_tree(&sc, sig, &sig_len, FALCON_SIG_CT, expkey,
			...
			&senct, &fsst, &dpkcpkt, &hmtapt, &ffsfft, &albt, &getlatpt, &compsigt);
		CYCLES_ADD_M(sgt)
	}
	
	total = senct + fsst + dpkcpkt + hmtapt + ffsfft + albt + getlatpt + compsigt;	
	pc.printf("sign start clocks:           %lld (%f%%)\n\r",fsst/BENCHMARK_ROUND,(double)((double)fsst/(double)total)*100);
	pc.printf("get deg/check params clocks: %lld (%f%%)\n\r",dpkcpkt/BENCHMARK_ROUND,(double)((double)dpkcpkt/(double)total)*100);
	pc.printf("hash mess to point clocks:   %lld (%f%%)\n\r",hmtapt/BENCHMARK_ROUND,(double)((double)hmtapt/(double)total)*100);
	pc.printf("sig encode clocks:           %lld (%f%%)\n\r",senct/BENCHMARK_ROUND,(double)((double)senc/(double)total)*100);
	pc.printf("apply lattice basis clocks:  %lld (%f%%)\n\r",albt/BENCHMARK_ROUND,(double)((double)albt/(double)total)*100);
	pc.printf("apply ff sampling clocks:    %lld (%f%%)\n\r",ffsfft/BENCHMARK_ROUND,(double)((double)ffsfft/(double)total)*100);
	pc.printf("get lattice point clocks:    %lld (%f%%)\n\r",getlatp/BENCHMARK_ROUND,(double)((double)getlatp/(double)total)*100);
	pc.printf("compute signature clocks:    %lld (%f%%)\n\r",compsigt/BENCHMARK_ROUND,(double)((double)compsigt/(double)total)*100);
	pc.printf("---------------------------------\n\r");
	pc.printf("total measured clocks:       %lld (%f%%)\n\r", total/BENCHMARK_ROUND,(double)((double)total/(double)total)*100);	
	pc.printf("---------------------------------\n\r");
	pc.printf("total sign t (%%meas) clocks: %lld (%f%%)\n\r", sgt/BENCHMARK_ROUND, (double)((double)(total/BENCHMARK_ROUND)/(double)(sgt/BENCHMARK_ROUND))*100);
	pc.printf("---------------------------------\n\r");

	pc.printf("Falcon failed if nonzero: %d\n\r", ret_val);
